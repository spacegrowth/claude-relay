...
| 29 | CAP | **Executor MCP policy** — executors launch with zero MCP servers (`--strict-mcp-config`); the packet's `MCP:` line (or `spawn --mcp`) opts in; `send` relaunches the same conversation with a wider set when a packet needs it (resume does NOT restore `--mcp-config`, proven by `tests/test_e2e_mcp.py`) ✅ LANDED on wake-fixes 2026-08-21 (unreleased — awaiting bump) | README "Executor MCP servers" |
| 30 | CAP | **Auto-close** — finished executors park themselves (landed = claimed files clean in the worktree, or idle past `auto_close_idle_minutes`); requires the lead to have surfaced the report; heavy → retire; `relay keep` pins ✅ LANDED on wake-fixes 2026-08-21 (unreleased — awaiting bump) | README "Auto-close" |
| 31 | CAP | **Executor agent** — executors launch as `--agent relay-executor` (inline from `agents/executor.md`): GATES/REPORT FORMAT in the system prompt, `Agent` tool removed, `git commit`/`push` denied via CLI `--disallowedTools` (holds under skip-perms; agent-level denies don't); packets carry only the per-packet footer ✅ LANDED on wake-fixes 2026-08-21 (unreleased — awaiting bump) | README "The executor agent" |
