...
| 32 | CAP | **Executor context window** — 200K default; `CONTEXT: 1m` packet line, explicit `--model x[1m]`, or relay's referenced-reading heuristic selects the 1M window; haiku excluded; retire seed hints 1m for heavy sessions ✅ LANDED v0.3.41 (2026-08-21) | README "Executor context window" |
| 33 | CAP | **Packet lint** — `relay lint` + advisory ⚠/ℹ at spawn/send: MCP mentioned-not-declared / unknown, big reading on 200K, CONTEXT/MCP unparsable, asks-to-commit, asks-to-ask, short packet, no Preconditions ✅ LANDED v0.3.41 (2026-08-21) | README Commands |
| 34 | CAP | **Tokens + launch in `relay list`** — `TOKENS` = real prompt/output from the transcript (dedup by message id, cached per size/mtime; `usage` in --json), `LAUNCH` = mcp/context/role ✅ LANDED v0.3.41 (2026-08-21) | README Commands |
| 35 | CAP | **`relay doctor`** — live proof that the installed CLI honours strict MCP / executor agent / commit deny / model aliases, + plumbing; `--offline`, `--quick`, `--json` ✅ LANDED v0.3.41 (2026-08-21) | README Troubleshooting |
| 36 | CAP | **`relay send --rotate`** — retire + seeded respawn (`<sid>-r2`, same territory, 1M when the tier has one) with the packet first; the heaviness gate points at it ✅ LANDED v0.3.41 (2026-08-21) | README Retiring |
| 37 | CAP | **relay board** — `/relay:board`: self-contained HTML overview (leads → executors → packet timelines with outcome/TL;DR/links, status/launch/tokens, warnings, copyable commands; light default + dark toggle; `lib/board_render.py` pure renderer, data from the same functions as `list`) ✅ LANDED v0.3.42 (2026-08-21) | README "The board" |
| 38 | CAP | **Model alias resolution** — spawn resolves `sonnet`/`opus`/`haiku`/`fable`(`[1m]`) through the local Claude Code (`system/init` model of a `-p` probe; unrecognised → refuse) and launches with the concrete id; cached per CLI version in `~/.relay-tasks/models.json`; fail-open on probe failure; doctor shows the cache ✅ LANDED v0.3.42 (2026-08-21) | README Config |
| 39 | CAP | **relay board redesign** — two-pane master/detail app (crisp warm-white + one cobalt accent), opens onto an executor, packet cards with full task/outcome, clean Status/Risk/Unverified grid, inline report/packet expanders, real diff link; `_read_capped` inline bodies ✅ LANDED v0.3.43 (2026-08-21) | README "The board" |
| 40 | CAP | **executor_default_context config** — the window an executor gets when nothing else decides it; shipped **1m** (window is a ceiling not consumption); packet `CONTEXT: 200k` opts down, haiku always 200k ✅ LANDED v0.3.44 (2026-08-21) | README Config |
| 41 | CAP | **Executor effort** — packet `EFFORT:` line / `spawn --effort` (low…max) → `claude --effort` at launch; per-process, re-passed on relaunch, changeable via resume/restart `--effort`, carried by `--rotate`; LAUNCH shows it; lint flags unparsable values ✅ LANDED v0.3.46 (2026-08-21) | README "Executor effort" |
| 42 | BUG | **Closed executors kept nagging their lead** — new_reports_for never checked status, so a closed/superseded executor whose surfaced stamp was refused (invoker gate) or whose older packets were never stamped re-announced forever; fix: closed/superseded silenced (dead still nags), close stamps ALL packet reports ✅ LANDED v0.3.46 (2026-08-21) | field report |
| 43 | CAP | **executor_fallback_model** — configured overload-fallback per executor via the per-launch --settings file (`fallbackModel` works interactively; the flag is print-only); user-chosen direction + visible notice, instead of wherever ✅ LANDED v0.3.47 (2026-08-22) | README Config |
| 44 | CAP | **relay stats** — one row per packet ever sent joining (model, effort) → rounds / verify verdict / report status, per-session token trailer, (model, effort) SUMMARY; `--json --lead --since` ✅ LANDED v0.3.48 (2026-09-01) | README "relay stats" |
| 45 | CAP | **executor heaviness on live context tokens** — `context_nudge_tokens` (150k) replaces the transcript-MB proxy (kept as fallback); `relay list` CTX column, TOKENS `·warm/·cold~N` (`cache_ttl_minutes`), `relay send` ctx/cache/hit-rate line, board hit-rate chip ✅ LANDED v0.3.48 (2026-09-01) | README Config, Cache state |
| 46 | DOC+CAP | **advisory nudges** — round-3 escalation note on send; `shape-haiku` / `shape-opus` / `reading-front-loaded` lint findings; effort rule rewritten as a quality lever, never a cost lever ✅ LANDED v0.3.48 (2026-09-01) | spawn skill, README |
| 47 | CAP | **landed ledger event** — per-packet landing recorded on send/check/sweep so `relay stats` ROUNDS ends on `landed` or `auto_commit` (manual lead commits were invisible before) ✅ LANDED v0.3.48 (2026-09-01) | README "relay stats" |
