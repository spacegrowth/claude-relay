...
| 41 | CAP | **Executor effort** — packet `EFFORT:` line / `spawn --effort` (low…max) → `claude --effort` at launch; per-process, re-passed on relaunch, changeable via resume/restart `--effort`, carried by `--rotate`; LAUNCH shows it; lint flags unparsable values ✅ LANDED on main 2026-08-21 (unreleased) | README "Executor effort" |
| 42 | BUG | **Closed executors kept nagging their lead** — new_reports_for never checked status, so a closed/superseded executor whose surfaced stamp was refused (invoker gate) or whose older packets were never stamped re-announced forever; fix: closed/superseded silenced (dead still nags), close stamps ALL packet reports ✅ LANDED on main 2026-08-21 (unreleased) | field report |
