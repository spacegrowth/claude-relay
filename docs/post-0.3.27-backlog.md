...
| 37 | CAP | **relay board** — `/relay:board`: self-contained HTML overview (leads → executors → packet timelines with outcome/TL;DR/links, status/launch/tokens, warnings, copyable commands; light default + dark toggle; `lib/board_render.py` pure renderer, data from the same functions as `list`) ✅ LANDED on main 2026-08-21 (unreleased) | README "The board" |
| 38 | CAP | **Model alias resolution** — spawn resolves `sonnet`/`opus`/`haiku`/`fable`(`[1m]`) through the local Claude Code (`system/init` model of a `-p` probe; unrecognised → refuse) and launches with the concrete id; cached per CLI version in `~/.relay-tasks/models.json`; fail-open on probe failure; doctor shows the cache ✅ LANDED on main 2026-08-21 (unreleased) | README Config |
